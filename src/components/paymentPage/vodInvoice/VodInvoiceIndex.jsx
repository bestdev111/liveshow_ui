...
class VodInvoiceIndex extends Component {
  state = {
    loadingInvoice: true,
    couponInputData: {
      vod_video_id: null,
  ...
  componentDidMount() {
    ...
    couponInputData["vod_video_id"] = this.props.location.state.vod_video_id;
  ...
  couponCodeApi = () => {
    api
      .postMethod("vod_videos_check_coupon_code", this.state.couponInputData)
      .then((response) => {
  ...
  payNow = (event) => {
    ...
    api
      .postMethod("vod_videos_payment_by_card", this.state.couponInputData)
      .then((response) => {
        if (response.data.success) {
          ToastContent(
            this.props.toastManager,
          ...
          this.props.history.push(
            `/vod/single/${this.props.location.state.title}`,
  ...
  paypalPaymentAPI = () => {
    api
      .postMethod("vod_videos_payment_by_paypal", this.state.couponInputData)
      .then((response) => {
        if (response.data.success) {
          ToastContent(
            this.props.toastManager,
...
export default withToastManager(translate(VodInvoiceIndex));